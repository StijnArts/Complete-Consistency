...
public class TagRegistry<T> {
    ...
    public static class Biomes extends Simple<Biome> {
        ...
        void add(TagKey<Biome> tagID, boolean optional, ResourceKey<Biome>... elements) {
            if (isFrozen) CompleteConsistency.LOGGER.warning("Adding Tag " + tagID + " after the API was frozen.");
            synchronized (this) {
    ...
    public void apply(Map<ResourceLocation, List<TagLoader.EntryWithSource>> tagsMap) {
        //this.isFrozen = true;
        if (CompleteConsistency.isDatagen()) {
    ...
    private static List<TagLoader.EntryWithSource> apply(
        ...
        ids.forEach(value -> builder.add(new TagLoader.EntryWithSource(value, CompleteConsistency.MOD_ID)));
        return builder;
    }
}
